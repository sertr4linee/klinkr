...
export class TransactionManager {
  ...
  private constructor(ttl: number = DEFAULT_TTL) {
    ...
    this.fileLock = FileLock.getInstance();
   ...
   */
  async preview(transactionId: string): Promise<string> {
    const transaction = this.getTransaction(transactionId);
  ...
  async commit(
    ...
    try {
      ...
      const afterSnapshot: FileSnapshot = {
        filePath,
  ...
  getActiveTransactions(): Transaction[] {
    return Array.from(this.transactions.values()).filter(
      tx => tx.status === 'pending' || tx.status === 'validated'
    );
  }
   ...
   */
  getTransactionsByFile(filePath: string): Transaction[] {
    return Array.from(this.transactions.values()).filter(
      tx => tx.realmId.sourceFile === filePath
    );
  }
  ...
  private startCleanup(): void {
    ...
    this.cleanupInterval = setInterval(() => {
      ...
      for (const [id, tx] of this.transactions) {
        ...
        if (
          (tx.status === 'committed' || tx.status === 'rolled_back' || tx.status === 'failed') &&
          age > 60 * 60 * 1000
        ) {
